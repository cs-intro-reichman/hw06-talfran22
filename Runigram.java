...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		// print(blend(tinypic, flippedHorizontally(tinypic), 0.5));
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		...
		for(int i = 0 ; i < image.length ; i++){
			for(int j = 0 ; j < image[0].length ; j++){
				newimage[i][image[0].length - 1 - j] =  new Color (image[i][j].getRGB());
			}
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for(int i = 0 ; i < image.length ; i++){
			for(int j = 0 ; j < image[0].length ; j++){
				newimage[image.length - 1 - i][j] =  new Color (image[i][j].getRGB());
			}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for(int i = 0 ; i < height ; i++){
			for(int j = 0 ; j < width ; j++){
				newimage[i][j] = new Color(image[i* row/height][j*col/width].getRGB());
			}
		}
		return newimage;
	}
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		...
		int red = (int)(Math.round((alpha * red1) + ((1 - alpha) * red2)));
		int green = (int)(Math.round((alpha * green1) + ((1 - alpha) * green2)));
		int blue = (int)(Math.round(((alpha * blue1) + ((1 - alpha) * blue2))));
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		Color newimage[][] = scaled(target, width, height);
		for(int i = 0 ; i <= n ; i++){
			display(blend(source, newimage, ((double)(n - i) / n)));
			StdDraw.pause(100);
		}
	}
